      ...
      {show && (
        <div className="modal">
          ...
          <div className="card">
            ...
            <p className="title">DOB: {convertDate(userDetails.registered.date)}</p>
            <p className="title">Age: {userDetails.registered.age}</p>
            <p className="title">Location: {userDetails.location.city}, {userDetails.location.country}</p>
            <p className="title">Post Code: {userDetails.location.postcode}</p>

            <p className="title">Phone: {userDetails.phone.replaceAll("-", "")}</p>
          </div>
          <button className="close-button topright" onClick={onHide}>X</button>
        </div>
      )}
    </>
  );
